...
@section('title', 'แก้ไขเหรียญตรา - ' . $badge->badge_name)
...
<div class="container">
    <div class="row mb-4">
        <div class="col-md-12">
            <div class="d-flex justify-content-between align-items-center">
                <h1 class="h3 mb-0">แก้ไขเหรียญตรา</h1>
                <div>
                    <a href="{{ route('admin.badges.show', $badge) }}" class="btn btn-outline-primary me-2">
    ...
    <div class="card shadow-sm">
        <div class="card-body">
            <form action="{{ route('admin.badges.update', $badge) }}" method="POST" enctype="multipart/form-data" id="badge-form" novalidate>
                @csrf
                @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <div class="field-container" id="badge_name-container">
                                <input type="text" class="form-control @error('badge_name') is-invalid @enderror"
                                       id="badge_name" name="badge_name" value="{{ old('badge_name', $badge->badge_name) }}" required
                        ...
                        <div class="mb-3">
                            ...
                            <div class="field-container">
                                <textarea class="form-control @error('badge_description') is-invalid @enderror"
                                         ...
                                         data-error-message="กรุณาระบุคำอธิบายเหรียญตรา">{{ old('badge_description', $badge->badge_description) }}</textarea>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <div class="field-container" id="type-container">
                                    <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required
                                        ...
                                        <option value="distance" {{ old('type', $badge->type) == 'distance' ? 'selected' : '' }}>ระยะทาง</option>
                                        <option value="calories" {{ old('type', $badge->type) == 'calories' ? 'selected' : '' }}>แคลอรี่</option>
                                        <option value="streak" {{ old('type', $badge->type) == 'streak' ? 'selected' : '' }}>ต่อเนื่อง</option>
                                        <option value="speed" {{ old('type', $badge->type) == 'speed' ? 'selected' : '' }}>ความเร็ว</option>
                                        <option value="event" {{ old('type', $badge->type) == 'event' ? 'selected' : '' }}>กิจกรรม</option>
                                    </select>
                                    <i class="fas fa-check-circle feedback-icon valid-feedback-icon"></i>
                                    <i class="fas fa-exclamation-circle feedback-icon invalid-feedback-icon"></i>
                                    @error('type')
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <div class="input-group has-validation">
                                    <input type="number" class="form-control @error('criteria') is-invalid @enderror"
                                           id="criteria" name="criteria" value="{{ old('criteria', $badge->criteria) }}" step="0.01" min="0" required
                            ...
                            <div class="text-center mb-3">
                                <div class="image-preview-container mx-auto mb-2">
                                    @if($badge->badge_image)
                                        <img src="{{ asset('storage/' . $badge->badge_image) }}" id="imagePreview" class="badge-img-preview">
